...
struct NotchMediaApp: App {
    ...
    var body: some Scene {
        MenuBarExtra("NotchMedia", systemImage: "sparkle", isInserted: $showMenuBarIcon) {
            Button("Settings") {
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func enableSkyLightOnAllWindows() {
        if Defaults[.showOnAllDisplays] {
            windows.values.forEach { window in
                if let skyWindow = window as? NotchMediaSkyLightWindow {
        ...
        Task {
            ...
            await MainActor.run {
                if Defaults[.showOnAllDisplays] {
                    ...
                    if let skyWindow = self.window as? NotchMediaSkyLightWindow {
                        skyWindow.disableSkyLight()
                    }
                }
            }
    ...
    private func createNotchMediaWindow(for screen: NSScreen, with viewModel: BoringViewModel) -> NSWindow {
        let rect = NSRect(x: 0, y: 0, width: windowSize.width, height: windowSize.height)
        let styleMask: NSWindow.StyleMask = [.borderless, .nonactivatingPanel, .utilityWindow, .hudWindow]
        
        let window = NotchMediaSkyLightWindow(contentRect: rect, styleMask: styleMask, backing: .buffered, defer: false)
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if !Defaults[.showOnAllDisplays] {
            ...
            let window = createNotchMediaWindow(
        ...
        if Defaults[.showOnAllDisplays] {
            ...
            for screen in NSScreen.screens {
                ...
                if windows[uuid] == nil {
                    ...
                    let window = createNotchMediaWindow(for: screen, with: viewModel)
            ...
            if window == nil {
                window = createNotchMediaWindow(for: selectedScreen, with: vm)
            }
